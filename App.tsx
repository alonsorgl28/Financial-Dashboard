...
import { getDashboardStats, getDebts, getTransactions, getBudgets, getCategories, addCategory, getScheduledPayments, addScheduledPayment, updateScheduledPayment, getPaymentConcepts, addPaymentConcept, getBtcContributions, addBtcContribution } from './services/dataService';
import { Transaction, TransactionCategory, DebtPriority, Debt, CategoryBudget, ScheduledPayment, BtcContribution } from './types';
import { ICONS } from './constants';

// Define the SectionPlaceholder component to handle state/empty views uniformly
...
const App: React.FC = () => {
  ...
  const [paymentConcepts, setPaymentConcepts] = useState(getPaymentConcepts());
  const [scheduledPayments, setScheduledPayments] = useState(getScheduledPayments());
  const [btcContributions, setBtcContributions] = useState(getBtcContributions());
  ...
  // Recalculate total debt whenever debts change
  useEffect(() => {
  ...
  const handleConfirmIncomeAdjustment = (newIncome: number, reason: string) => {
    setStats(prev => {
      ...
      return {
        ...
        availableCash: prev.availableCash + diff // Sync available cash with income change delta
      };
    });
  ...
  const handleConfirmNewPaymentConcept = (name: string) => {
    const updated = addPaymentConcept(name);
  ...
  const handleConfirmScheduledPayment = (data: Partial<ScheduledPayment>) => {
    if (editingScheduledPayment) {
      const updated = updateScheduledPayment(editingScheduledPayment.id, data);
      if (updated) {
        setScheduledPayments([...getScheduledPayments()]);
        setToast({ message: 'Pago programado actualizado correctamente', type: 'success' });
      }
    } else {
      const created = addScheduledPayment(data as Omit<ScheduledPayment, 'id'>);
      setScheduledPayments([...getScheduledPayments()]);
  ...
  const handleConfirmBtcContribution = (contribution: Omit<BtcContribution, 'id'>) => {
    const created = addBtcContribution(contribution);
    setBtcContributions([created, ...btcContributions]);
    ...
    // Register as a transaction too
    const btcTx: Transaction = {
      id: Math.random().toString(36).substr(2, 9),
      date: created.date,
      description: `Inversión BTC: ${created.notes || 'Aporte'}`,
      amount: created.amount,
      category: 'Inversión', // Using a generic category or we could add 'Inversión' to TransactionCategory
      isWeekend: false,
      status: 'categorized'
    };
    setTransactions([btcTx, ...transactions]);
  ...
  const handleSaveTransaction = (data: any) => {
    if (editingTransaction && editingTransaction.id) {
      ...
      const diff = updatedTx.amount - editingTransaction.amount;
      setStats(prev => ({
        ...prev,
        availableCash: prev.availableCash - diff,
        weekendSpent: updatedTx.isWeekend ? prev.weekendSpent + diff : (editingTransaction.isWeekend ? prev.weekendSpent - editingTransaction.amount : prev.weekendSpent)
      }));
    } else {
      ...
      setStats(prev => ({
        ...prev,
        availableCash: prev.availableCash - newTx.amount,
        weekendSpent: newTx.isWeekend ? prev.weekendSpent + newTx.amount : prev.weekendSpent
      }));
      if (newTx.category === TransactionCategory.DEBT) {
        setDebts(prevDebts => prevDebts.map(d => {
          if (newTx.description.includes(d.name) || (d.priority === 1 && newTx.description.toLowerCase().includes('scheu'))) {
            return { ...d, currentBalance: d.currentBalance - newTx.amount };
          }
          return d;
        }));
      }
                  ...
                  <tr key={tx.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                        tx.category === TransactionCategory.WEEKEND ? 'bg-amber-100 text-amber-700' :
  ...
  return (
    <Layout activeTab={activeTab} setActiveTab={setActiveTab}>
                ...
                <div key={debt.id} className={`${cardStyles} rounded-3xl p-8 border relative overflow-hidden transition-all hover:translate-y-[-4px]`}>
                  ...
                  <div className="space-y-6 mt-8">
                    ...
                    <button 
                      ...
                      className={`w-full py-3 mt-2 rounded-xl font-bold text-sm transition-all border ${
                        isP1 ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-100 hover:bg-indigo-700' :
      ...
      {toast && (
        <div className={`fixed bottom-8 right-8 z-[100] flex items-center space-x-3 px-6 py-4 rounded-2xl shadow-2xl animate-in slide-in-from-right-full ${
