...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      {/* Sidebar - Responsive */}
      <aside className={`
        fixed md:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col shrink-0 transition-transform duration-300 ease-in-out
          ...
          {navItems.map((item) => (
            <button
              ...
              onClick={() => {
                ...
                setIsSidebarOpen(false); // Close on click (mobile)
              }}
      ...
      <main className="flex-1 flex flex-col overflow-hidden w-full">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold capitalize truncate max-w-[150px] md:max-w-none">
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50/50 scroll-smooth">
          {children}
        </div>
      </main>
    </div>
