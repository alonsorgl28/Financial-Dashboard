...
interface EditWeekendCapModalProps {
  isOpen: boolean;
  onClose: () => void;
  currentCap: number;
  onConfirm: (newCap: number, reason: string) => void;
}

const EditWeekendCapModal: React.FC<EditWeekendCapModalProps> = ({ isOpen, onClose, currentCap, onConfirm }) => {
  const [newCap, setNewCap] = useState<number>(currentCap);
  const [reason, setReason] = useState<string>('');
  const currentMonthName = new Intl.DateTimeFormat('es-ES', { month: 'long', year: 'numeric' }).format(new Date());

  useEffect(() => {
    if (isOpen) {
      setNewCap(currentCap);
      setReason('');
    }
  }, [isOpen, currentCap]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (newCap < 0 || !reason.trim()) return;
    onConfirm(newCap, reason);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Editar Cap de Fin de Semana</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <div className="flex justify-between items-center text-sm mb-1">
              <span className="text-slate-500">Cap Actual</span>
              <span className="font-bold text-slate-900">S/{currentCap.toLocaleString()}</span>
            </div>
          </div>

          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Nuevo Cap Mensual (S/.)</label>
              <input 
                type="number" 
                step="0.01"
                min="0"
            ...
            <div>
              ...
              <textarea 
                required
                rows={2}
                placeholder="Ejem: Evento especial / ajuste de presupuesto"
                value={reason}
          ...
          <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl text-xs text-amber-800 leading-relaxed">
            <p className="font-bold mb-1 uppercase tracking-wider text-[10px]">⚠️ Aviso</p>
            Este cambio actualiza el límite mensual de fines de semana. No se registrará como gasto ni transacción en el historial.
          </div>
...
export default EditWeekendCapModal;
